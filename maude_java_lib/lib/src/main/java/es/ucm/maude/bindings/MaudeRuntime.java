...
public class MaudeRuntime {
    ...
    public static final String MAUDE_STDLIB_RESOURCE_PREFIX = "maude/stdlib/";
    ...
    private synchronized void loadNativeLibraries() {
        ...
        if (tempDir == null) {
            throw new IllegalStateException(
                "Temporary directory must be created before loading native libraries"
            );
        }
     ...
     */
    private File extractLibrary(String resourcePath, String libraryName)
        throws IOException {
     ...
     */
    public synchronized void loadFromResources(
        String maudeProgramResourcePath
    ) {
        if (tempDir == null) {
            throw new IllegalStateException(
                "Temporary directory must be created before loading modules"
            );
        }
        ...
        if (inputStream == null) {
            throw new RuntimeException(
                "Maude module not found in JAR: " + maudeProgramResourcePath
            );
        }
        ...
        try (FileOutputStream outputStream = new FileOutputStream(outputFile)) {
            ...
            throw new RuntimeException(
                "Failed to extract Maude source file: " +
                    maudeProgramResourcePath,
                e
            );
        } finally {
            try {
                ...
                logger.warning(
                    "Failed to close input stream for Maude source file: " +
                        maudeProgramResourcePath
                );
            }
        }
    ...
    public synchronized void loadMaudeStdlib() {
        if (tempDir == null) {
            throw new IllegalStateException(
                "Temporary directory must be created before loading standard library"
            );
        }
